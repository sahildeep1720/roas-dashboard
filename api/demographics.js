...
export default async function handler(req, res) {
  ...
  const { breakdown = 'gender', days = 7, from, to, campaign_id, adset_id } = req.query;
  if (!VALID.includes(breakdown)) return res.status(400).json({ error: 'Invalid breakdown' });
  ...
  const url = `https://graph.facebook.com/v19.0/act_${META_ACCOUNT}/insights?fields=${fields}&time_range={"since":"${since}","until":"${until}"}&breakdowns=${breakdown}&level=account&limit=100&access_token=${META_TOKEN}${filterParam}`;

  try {
    const response = await fetch(url);
    const data = await response.json();
    if (data.error) return res.status(400).json({ error: data.error.message });
    return res.status(200).json({ data: data.data || [], breakdown });
  } catch (err) {
